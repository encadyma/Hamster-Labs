...
class virtual_world:
    ...
    def get_robot_points(self, a, x, y):
        canvas_width = self.canvas_width
        canvas_height = self.canvas_height
        pi4 = 3.1415 / 4  # quarter pi
        vrobot = self.vrobot
    ...
    def in_collision(self, a, x, y):
        # SAT COLLISION
        # Check two sides instead of four.
        # https://www.sevenson.com.au/actionscript/sat/

        robot_points = self.get_robot_points(a, x, y)

        rpt_1 = robot_points[0]
        rpt_2 = robot_points[1]
        rpt_3 = robot_points[2]
        rpt_4 = robot_points[3]

        axis_1 = (rpt_1[0] - rpt_2[0], rpt_1[1] - rpt_2[1])
        axis_2 = (rpt_2[0] - rpt_3[0], rpt_2[1] - rpt_3[1])

        for obstacle in self.map:
            opt_tl = (obstacle[0], obstacle[1])
            opt_tr = (obstacle[2], obstacle[1])
            opt_bl = (obstacle[0], obstacle[3])
            opt_br = (obstacle[2], obstacle[3])
